...
async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    if (renderServerComponentData) {
        return new _renderResult.default((res_, next)=>{
            const { startWriting  } = connectReactServerReadableStreamToPiper(res_.write, next);
            startWriting((0, _writerBrowserServer).renderToReadableStream(/*#__PURE__*/ _react.default.createElement(OriginalComponent, Object.assign({
            }, props)), serverComponentManifest).getReader());
        });
    }
    ...
    const renderDocument = async ()=>{
        ...
        if (!process.browser && Document.getInitialProps) {
            ...
            const bodyResult = async ()=>{
                ...
                return concurrentFeatures ? process.browser ? await renderToWebStream(content) : await renderToNodeStream(content, generateStaticHTML) : piperFromArray([
...
function connectReactServerReadableStreamToPiper(write, next) {
    let bufferedString = '';
...
function renderToWebStream(element) {
    return new Promise((resolve, reject)=>{
        ...
        const { startWriting  } = connectReactServerReadableStreamToPiper((s)=>{
            if (!underlyingStream) {
                throw new Error('invariant: `write` called without an underlying stream. This is a bug in Next.js');
            }
            ...
            if (!underlyingStream) {
                throw new Error('invariant: `next` called without an underlying stream. This is a bug in Next.js');
            }
            underlyingStream.next(err);
        });
        const reader = _server.default.renderToReadableStream(element, {
            onError (err) {
                if (!resolved) {
                    resolved = true;
                    reject(err);
