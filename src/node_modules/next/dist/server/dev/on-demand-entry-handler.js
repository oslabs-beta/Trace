...
function onDemandEntryHandler(watcher, multiCompiler, { pagesDir , nextConfig , maxInactiveAge , pagesBufferLength  }) {
    ...
    return {
        async ensurePage (page, clientOnly) {
            ...
            if (entriesChanged) {
                (0, _output).reportTrigger(isApiRoute ? `${normalizedPage} (server only)` : isMiddleware ? `${normalizedPage} (middleware only)` : clientOnly ? `${normalizedPage} (client only)` : normalizedPage);
